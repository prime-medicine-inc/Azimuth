...
def gene_feature(Y, X, learn_options):
    ...
    for gene in gene_names.unique():
        ...
        temperature[gene_names.values==gene] = Tm.Tm_NN(seq, nn_table='RNA_NN2')
...
def Tm_feature(data, pam_audit=True, learn_options=None):
    ...
    for i, seq in enumerate(sequence):
        if pam_audit and seq[25:27]!="GG":
            raise Exception("expected GG but found %s" % seq[25:27])
        rna = False
        featarray[i,0] = Tm.Tm_NN(seq, nn_table='RNA_NN2')        #30mer Tm
        featarray[i,1] = Tm.Tm_NN(seq[segments[0][0]:segments[0][1]], nn_table='RNA_NN2') #5nts immediately proximal of the NGG PAM
        featarray[i,2] = Tm.Tm_NN(seq[segments[1][0]:segments[1][1]], nn_table='RNA_NN2')   #8-mer
        featarray[i,3] = Tm.Tm_NN(seq[segments[2][0]:segments[2][1]], nn_table='RNA_NN2')      #5-mer
